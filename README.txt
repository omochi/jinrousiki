---------------------------------------------------------------------------------
  人狼式 〜汝は人狼なりや？式神エディション Version 1.4.0 β6 README 2010/03/12
---------------------------------------------------------------------------------
...
------------
  動作環境
------------
本スクリプトは、PHP4＋MySQL4.0 環境下での動作を想定しています。
...
1. setup.php       : 初期設定を行うスクリプト。
                     新しくサーバに設置した際に、初めに実行すること。
2. room_delete.php : 稼働中の村を削除するためのスクリプト。
                     include/init.php 中の $DEBUG_MODE が true の時のみ有効

room_delete.php については直接呼び出すものではなく、稼働中の村一覧に表示される
削除用リンクから呼び出されるものです。
ただし、admin/ ディレクトリにアクセス制限をかけていない場合、
かつ $DEBUG_MODE が true の時には直接アドレス入力することで、
任意の村の削除が誰にでも可能となります。
そのため、通常稼動時には $DEBUG_MODE を false に設定しておくようにしてください。
...
2. game_config.php   : 配役、制限時間など、ゲームパラメータの設定ファイル
3. message.php       : ゲーム中で表示されるシステムメッセージの設定ファイル
4. version.php       : トップページで表示されるバージョン情報の設定ファイル
...
また、村が稼動している間、村終了後も会話しているプレイヤーがいる間は、salt 文字列
を変更しないようにしてください。途中で salt 文字列が変更されるとログインができなく
なります。
