...
ゲーム中の仮想時間 (昼12時間、夜6時間) を経過した後に [<?= $TIME_CALC->sudden_death ?>] 経過すると投票していない人は自動で突然死となります。<br>
突然死が発生するとその日の投票がリセットされて再投票となります、注意してください。</div>


<h2>ユーザの似顔絵などを表すユーザアイコンを自由にアップロードできます</h2>
<div class="diff"><a href="icon_upload.php">専用のページ</a>から [<?= $USER_ICON->IconSizeMax() . '、容量 ' . $USER_ICON->IconFileSizeMax() ?>] のファイルをアップロードできます。</div>


<h2>自動廃村</h2>
<div class="diff">ゲームが開始されない場合、最後に発言された時間から [<?= $TIME_CALC->die_room ?>] 放置されると自動で村は廃墟になります。<br>
...
[<?= $GAME_CONF->draw ?>回] 再投票が続いた場合は自動的に引き分けとなり、ゲームは終了します。
</div>
...
<?= "output_diff_time('" . TZDate('Y, m, j, G, i, s', TZTime()) . "');" ?>
</script>秒</span>
</div>
...
現在の設定は [ <?= ($GAME_CONF->poison_only_voter ? '投票者' : '生存者全員') ?>からランダム ] です。
</div>


<h2>人狼が埋毒者を襲撃した際に巻き添えになる対象を限定可能</h2>
<div class="diff">サーバ管理者がゲーム設定を変更する事で人狼が埋毒者を襲撃した際に巻き添えになる対象を限定する事が可能です。<br>
現在の設定は [ <?= ($GAME_CONF->poison_only_eater ? '襲撃者固定' : '人狼全員からランダム') ?> ] です。
</div>
...
現在の設定は [ 登録<?= ($GAME_CONF->entry_one_ip_address ? "不可" : "可能") ?> ] になっています。
</div>
...
<h2>強制沈黙</h2>
<div class="diff">非リアルタイム制の場合、誰も発言をせず <?= $TIME_CALC->silence ?> 過ぎた場合には強制的に沈黙となり時間が消費されます。<br>
消費される時間は 昼： <?= $TIME_CALC->silence_day ?> 夜： <?= $TIME_CALC->silence_night ?>です。
</div>


<h2>非リアルタイム制の会話の時間消費の上限</h2>
<div class="diff">半角100文字(全角50文字)で 昼： <?= $TIME_CALC->spend_day ?> 夜：<?= $TIME_CALC->spend_night ?> ずつ消費されていきますが、どれだけ文字が増えても最大半角400文字(全角200文字)までの消費時間までしか増えません。<br>
...
<?= $GAME_CONF->objection ?> 回「異議あり」を使用すると二度と使えなくなります。
</div>
